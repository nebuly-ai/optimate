...
class Optimizer:
                ...
                for nextNode in node.outputs[0].outputs:
                    if nextNode.op == "Mul":
                        if swishOutputTensor is None:
                            ...
                            assert nextGemmNode.op == "Gemm", (
                                "Unexpected node type for nextGemmNode "
                                "{}".format(nextGemmNode.name)
                            )
                            nextGemmNode.inputs = [
                                swishOutputTensor,
    ...
    def resize_fix(self):
        """
        This function loops through the graph looking for Resize
        nodes that uses scales for resize (has 3 inputs).
        It substitutes found Resize with Resize that takes the
        size of the output tensor instead of scales.
        It adds Shape->Slice->Concat
                Shape->Slice----^     subgraph to the graph to
    ...
    def adjustAddNode(self):
        nAdjustAddNode = 0
        for node in self.graph.nodes:
            # Change the bias const to the second input to
            # allow Gemm+BiasAdd fusion in TRT.
            if node.op in ["Add"] and isinstance(
                node.inputs[0], gs.ir.tensor.Constant
            ):
    ...
    def insert_groupnorm_plugin(self):
        ...
        for node in self.graph.nodes:
            if (
                ...
                and node.o().o().o().o().o().o().o().o().o().o().o().op
                == "Mul"
                and node.o().o().o().o().o().o().o().o().o().o().o().o().op
                == "Add"
                and len(
                    node.o().o().o().o().o().o().o().o().inputs[1].values.shape
                )
                == 3
            ):
                ...
                gamma = np.array(
                    deepcopy(gammaNode.inputs[index].values.tolist()),
                    dtype=np.float32,
                )
                constantGamma = gs.Constant(
                    "groupNormGamma-" + str(nGroupNormPlugin),
                    np.ascontiguousarray(gamma.reshape(-1)),
                )  # MUST use np.ascontiguousarray, or TRT will regard the
                # shape of this Constant as (0) !!!
                ...
                beta = np.array(
                    deepcopy(betaNode.inputs[index].values.tolist()),
                    dtype=np.float32,
                )
                ...
                groupNormN = gs.Node(
                    ...
                    outputs=[groupNormV],
                    attrs=OrderedDict(
                        [("epsilon", epsilon), ("bSwish", int(bSwish))]
                    ),
                )
    ...
    def insert_layernorm_plugin(self):
        ...
        for node in self.graph.nodes:
            if (
                ...
                and len(
                    node.o().o(0).o().o().o().o().o().inputs[1].values.shape
                )
                == 1
            ):
                ...
                # The first axis to normalize from can be inferred
                # from the size of the `axes`
                # parameter of (any of) the `ReduceMean` node(s)
                reduceMeanNode = node.o().o(0).o()
                assert reduceMeanNode.op == "ReduceMean"
                firstNormAxis = -1 * np.size(
                    np.array(reduceMeanNode.attrs["axes"])
                )
                ...
                constantGamma = gs.Constant(
                    ...
                    np.ascontiguousarray(gamma.reshape(-1)),
                )  # MUST use np.ascontiguousarray, or TRT will
                # regard the shape of this Constant as (0) !!!
                ...
                layerNormN = gs.Node(
                    ...
                    inputs=inputList,
                    attrs=OrderedDict(
                        [("epsilon", 1.0e-5), ("axis", firstNormAxis)]
                    ),
                    outputs=[layerNormV],
                )
    ...
    def insert_seq2spatial_plugin(self):
        ...
        for node in self.graph.nodes:
            if node.op == "Transpose" and node.o().op == "Conv":
                ...
                ), "Unexpected node type for reshapeNode {}".format(
                    reshapeNode.name
                )
                residualNode = reshapeNode.i(0)
                assert (
                    residualNode.op == "Add"
                ), "Unexpected node type for residualNode {}".format(
                    residualNode.name
                )
                ...
                outputShapeTensor = (
                    transposeNode.i().i().i(1).i(1).i(1).i().inputs[0]
                )
    ...
    def fuse_kv(self, node_k, node_v, fused_kv_idx, heads, num_dynamic=0):
        ...
        # Concat and interleave weights such that the output
        # of fused KV GEMM has [b, s_kv, h, 2, d] shape
        ...
        constant_weights_kv = gs.Constant(
            "Weights_KV_{}".format(fused_kv_idx),
            np.ascontiguousarray(weights_kv),
        )
        ...
        # Connect the output of fused node to the inputs of
        # the nodes after K and V
    ...
    def insert_fmhca(
        ...
        if num_dynamic > 0:
            reshape2_input1_out = gs.Variable(
                "Reshape2_fmhca{}_out".format(mhca_idx),
                np.dtype(np.int64),
                None,
            )
    ...
    def fuse_qkv(
        ...
        # Concat and interleave weights such that the output
        # of fused QKV GEMM has [b, s, h, 3, d] shape
        ...
        output_tensor_k = node_k.outputs[0]
        # Concat and interleave weights such that the output of
        # fused QKV GEMM has [b, s, h, 3, d] shape
        constant_weights_qkv = gs.Constant(
            "Weights_QKV_{}".format(fused_qkv_idx),
            np.ascontiguousarray(weights_qkv),
        )
        ...
        # Connect the output of the fused node to the
        # inputs of the nodes after Q, K and V
    ...
    def mha_mhca_detected(self, node, mha):
        ...
        # If we are looking for MHCA inputs of two matmuls (K and V)
        # must be equal.
        ...
        if (
            ...
            if node.o().op == "Shape":
                if node.o(1).op == "Shape":
                    ...
                    num_dynamic_kv = 1
                # For Cross-Attention, if batch axis is dynamic (in QKV),
                # assume H*W (in Q) is dynamic as well
    ...
    def fuse_kv_insert_fmhca(self, heads, mhca_index, sm):
        ...
        for idx, _ in enumerate(nodes):
            # fMHCA can't be at the 2 last layers of the network.
            # It is a guard from OOB
            if idx + 1 > len(nodes) or idx + 2 > len(nodes):
                continue

            # Get anchor nodes for fusion and fMHCA plugin
            # insertion if the MHCA is detected
            (
                detected,
                num_dynamic_q,
                num_dynamic_kv,
            ...
            if detected:
                assert (
                    num_dynamic_q == 0 or num_dynamic_q == num_dynamic_kv + 1
                )
                # Skip the FMHCA plugin for SM75 except for when the
                # dim per head is 40.
    ...
    def fuse_qkv_insert_fmha(self, heads, mha_index):
        ...
        for idx, _ in enumerate(nodes):
            # fMHA can't be at the 2 last layers of the network.
            # It is a guard from OOB
            if idx + 1 > len(nodes) or idx + 2 > len(nodes):
                continue

            # Get anchor nodes for fusion and fMHA plugin insertion
            # if the MHA is detected
            (
                detected,
                num_dynamic_q,
                num_dynamic_kv,
...
class UNet(BaseModel):
    ...
    def optimize(self, onnx_graph, minimal_optimization=False):
        ...
        if bMHAPlugin and not bDisablePlugins:
            ...
            opt.info(
                "UNet: inserted " + str(num_fmha_inserted) + " fMHA plugins"
            )
        ...
        if bMHCAPlugin and not bDisablePlugins:
            ...
            opt.info(
                "UNet: inserted " + str(num_fmhca_inserted) + " fMHCA plugins"
            )
